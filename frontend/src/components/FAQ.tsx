  ...
  useEffect(() => {
    const fetchFAQs = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch(`${API_BASE}/api/faq?lang=${currentLang}`);
        ...
        const fetchedFAQs = Array.isArray(data.faqs) 
          ? data.faqs.sort((a: FAQItem, b: FAQItem) => a.order - b.order)
          : [];
